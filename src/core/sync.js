...
export function describeError(err) {
  ...
  // Ответа нет вовсе — запрос не дошёл. Опираться тут на navigator.onLine
  // бессмысленно: он показывает «онлайн» и при мёртвом соединении, лишь бы
  // была сеть. Отсутствие статуса — куда более надёжный признак.
  if (!err?.status) {
    return (
      'Запрос не дошёл до GitHub. Обычно это обрыв связи, блокировщик ' +
      'в браузере или VPN. Прогресс на месте — синхронизация повторится сама.'
    );
  }
