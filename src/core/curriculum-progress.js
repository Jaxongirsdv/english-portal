...
import { isLessonMastered, lessonScore, lessonStage } from './lesson-progress.js';
...
export function levelProgress(state, level) {
  ...
  const attempted = lessons.filter((lesson) => state.lessons?.[lesson.id]);
  const completed = attempted.filter((lesson) => ['completed', 'mastered'].includes(lessonStage(state.lessons[lesson.id])));
  const mastered = attempted.filter((lesson) => isLessonMastered(state.lessons[lesson.id]));
  const scores = attempted.map((lesson) => lessonScore(state.lessons[lesson.id]));
  return {
    attempted: attempted.length,
    completed: completed.length,
    mastered: mastered.length,
    // Старое имя оставлено для экранов и внешнего прогресса: done теперь означает освоено.
    done: mastered.length,
    total: lessons.length,
    average: scores.length ? Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length) : 0,
    complete: lessons.length > 0 && mastered.length === lessons.length,
  };
}
...
export function isLessonUnlocked(state, lessonId) {
  ...
  if (state.milestones?.[level.id]?.passed) return true;
  const lessons = lessonsForLevel(level);
  const index = lessons.findIndex((item) => item.id === lessonId);
  return index === 0 || isLessonMastered(state.lessons?.[lessons[index - 1].id]);
}
...
export function isMilestoneUnlocked(state, levelId) {
  ...
  return !!level
    ...
    && (!!state.milestones?.[levelId]?.passed || levelProgress(state, level).complete);
}

export function nextCurriculumStep(state) {
  for (const level of CURRICULUM) {
    if (!isLevelUnlocked(state, level)) continue;
    if (state.milestones?.[level.id]?.passed) continue;
    const lesson = lessonsForLevel(level).find((item) => !isLessonMastered(state.lessons?.[item.id]));
