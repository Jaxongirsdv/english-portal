...
test('оборванный запрос объясняется по-человечески', () => {
  // fetch бросает TypeError без статуса — «Failed to fetch» само по себе
  // ничего не подсказывает, поэтому подменяем его понятной причиной
  const text = describeError(new TypeError('Failed to fetch'));

  assert.ok(!text.includes('Failed to fetch'), 'системный текст показывать бесполезно');
  assert.ok(/связ|блокиров|VPN/i.test(text), 'должно быть видно, где искать причину');
  assert.ok(/прогресс/i.test(text), 'и что прогресс при этом цел');
});
